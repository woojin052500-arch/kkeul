...
# We know the top part is fine up to `return (`
idx_return = current.find('return (')
if idx_return == -1:
    print("Error: Could not find return statement")
    exit()

top_part = current[:idx_return]

# Now we will manually construct the entire `return (` block and write it.
return_block = """return (
    <div style={{ 
      padding: '20px 24px', 
      display: 'flex', 
          ...
          <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
            ...
            <div className="toss-input-group">
              <input
                ...
                autoFocus
              />
              <label className="toss-input-placeholder">학교명</label>
            </div>
          </div>
        )}

        {/* STEP 9: Interests (Student) */}
        {step === 9 && mode === 'signup' && (
          <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
              {['IT/개발', '창업', '수학/과학', '예술/문화', '인문학', '체육', '봉사활동', '외국어/어학'].map(interest => (
                <button
                  key={interest}
                  onClick={() => {
                    setInterests(prev => 
